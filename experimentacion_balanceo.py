...
def train_cv_model(pipeline_aux, X, y, nombre, balanceador, n_splits=10):
    """
    ...
    registro_df.to_csv(f'./resultados/balanceo/{balanceador}/training/resultados_{nombre}_{balanceador}.csv', index=False)
...
def reporte_metricas(nombre, balanceador, y_true, y_pred, y_prob=None, pos_label=1):
    ...
    resultados_testing_generales_df.to_csv(f'./resultados/balanceo/{balanceador}/testing/resultados_generales_{nombre}_{balanceador}.csv', index=False)
    ...
    plt.tight_layout()
    plt.savefig(f'./resultados/balanceo/{balanceador}/testing/confusion_matrix_{nombre}_{balanceador}.png')
    plt.close()
    ...
    resultados_testing_clases_df.to_csv(f'./resultados/balanceo/{balanceador}/testing/resultados_clases_{nombre}_{balanceador}.csv', index=False)
...
for nombre, modelo in modelos_base.items():
    models_smote[nombre] = Pipeline([
        ("smote", SMOTE(sampling_strategy=0.7, random_state=42)),
        ("clf", modelo)
    ])

for nombre, pipeline in models_smote.items():
    print(f"\n=== Entrenando {nombre} ===")

    models_smote[nombre], resultados[nombre+'Smote'] = train_cv_model(pipeline, X_train, y_train, nombre, 'smote')
    print(models[nombre], resultados[nombre+'Smote'])
...
for name,model in models_smote.items():
    ...
    if name != 'SVM':
        ...
        y_proba = None
    reporte_metricas(name, 'smote', y_val, y_pred, y_prob=y_proba)
    print("\n")